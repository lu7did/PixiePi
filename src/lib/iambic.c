...
static int cw_keyer_mode = KEYER_STRAIGHT;
...
void keyer_event(int gpio, int level, uint32_t tick) {
    ...
    if (gpio == GPIO_LEFT) {
     ...
     {   kcwr = state;
         (TRACE>=0x02 ? fprintf(stderr,"%s:keyer_event: RIGHT_PADDLE event detected\n",PROGID) : _NOP);
     }

    if (state || cw_keyer_mode == KEYER_STRAIGHT) {
        sem_post(&cw_event);
        (TRACE>=0x03 ? fprintf(stderr,"%s:keyer_event: STRAIGHT Condition\n",PROGID) : _NOP);
    }
}
//*---------------------------------------------------------------------------------------------
//* clear_memory
...
void set_keyer_out(int state) {
    ...
    if (keyer_out != state) {
        ...
        if (state) {
            ...
            (TRACE>=0x02 ? fprintf(stderr,"%s:set_keyer_out: PTT set to true\n",PROGID) : _NOP);
            setPTT(true);
        }
        else {
            keyState=KEY_UP;                           //* The keyer has been released or the PTT deactivated
            if (keyChange!=NULL) { keyChange(); }      //* First execute callback
            (TRACE>=0x02 ? fprintf(stderr,"%s:set_keyer_out: PTT set to false\n",PROGID) : _NOP);
            setPTT(false);
        }
    }
}
...
static void* keyer_thread(void *arg) {
    ...
    setPTT(false);
    (TRACE>=0x02 ? fprintf(stderr,"%s:keyer_thread: PTT set to false\n",PROGID) : _NOP);
    return 0 ;
}
//*------------------------------------------------------------------------------------------------
//* iambic_close
...
void iambic_close() {
    ...
    //sem_post(&cw_event);
    running=false;
    (TRACE>=0x02 ? fprintf(stderr,"%s:iambic_close: Posting terminating event\n",PROGID) : _NOP);


    sem_post(&cw_event);
    (TRACE>=0x02 ? fprintf(stderr,"%s:iambic_close: Stopping keyer thread\n",PROGID) : _NOP);

    pthread_join(keyer_thread_id, 0);
    (TRACE>=0x02 ? fprintf(stderr,"%s:iambic_close: Killing semaphore\n",PROGID) : _NOP);
...
int iambic_init()  {
    ...
    (TRACE>=0x02 ? fprintf(stderr,"%s:iambic_init: Initializing Keyer thread\n",PROGID) : _NOP);

    if(gpioInitialise()<0) {
       (TRACE>=0x00 ? fprintf(stderr,"%s:iambic_init: Cannot initialize GPIO\n",PROGID) : _NOP);
        return -1;
    }
    ...
    gpioSetAlertFunc(GPIO_RIGHT, keyer_event);

    gpioSetMode(GPIO_LEFT, PI_INPUT);
    gpioSetPullUpDown(GPIO_LEFT,PI_PUD_UP);
    usleep(100000);

    gpioSetAlertFunc(GPIO_LEFT, keyer_event);
    ...
    if (wiringPiSetup () < 0) {
       (TRACE>=0x00 ? fprintf(stderr,"%s:iambic_init: Unable to setup wiringPi\n",PROGID) : _NOP);
        return 1;
    }
    int i=0;
    ...
    if (i < 0) {
       (TRACE>=0x00 ? fprintf(stderr,"%s:iambic_init: Failed to create thread\n",PROGID) : _NOP);
        exit(-1);
    }
    (TRACE>=0x00 ? fprintf(stderr,"%s:iambic_init: Keyer ready\n",PROGID) : _NOP);
    return 0;
}
