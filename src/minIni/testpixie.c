...
/*  printf("1. String reading tests passed\n");
  ...
  /* value reading */
  n = ini_getl("VFO", "VFO_START", 7000000, inifile);
  assert(n==7000000);
  n = ini_getl("VFO", "VFO_END", 7200000, inifile);
  assert(n==7200000);
  n = ini_getl("VFO", "VFOA", 7032000, inifile);
  assert(n==7032000);
  char hi[80];
  sprintf(hi,"%d",7100000);
  n = ini_puts("VFO","VFOC",hi,inifile);
  n = ini_gets("VFO", "VFOC", "0", hi, sizearray(hi), inifile);
  printf("Read key VFOC=%s",hi);
